...
public class EqZeroGameTarget : TargetRules
{
	public EqZeroGameTarget(TargetInfo Target) : base(Target)
	{
	...
	internal static void ApplySharedEqZeroTargetSettings(TargetRules Target)
	{
		...
		if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
		{
			...
			EqZeroGameTarget.ConfigureGameFeaturePlugins(Target);
		}
		else
		{
			// !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
			...
			else
			{
				...
				if (!bHasWarnedAboutShared)
				{
					...
					Logger.LogWarning("EqZeroGameEOS and dynamic target options are disabled when packaging from an installed version of the engine");
				}
			}
		}
	}
