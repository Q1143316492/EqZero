...
namespace EqZeroGameplayTags
{
	...
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(EqZero_Anim_Debug, "EqZero.Anim.Debug", "Debug messages from Animation Instance.");
	...
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(Gameplay_Message_ADS, "Gameplay.Message.ADS", "Message broadcast when ADS state changes.");
	...
	FGameplayTag FindTagByString(const FString& TagString, bool bMatchPartialString)
	{
		...
		if (!Tag.IsValid() && bMatchPartialString)
		{
			...
			for (const FGameplayTag& TestTag : AllTags)
			{
				...
				{
					UE_LOG(LogEqZero, Display, TEXT("Could not find exact match for tag [%s] but found partial match on tag [%s]."), *TagString, *TestTag.ToString());
					Tag = TestTag;
					break;
				}
			}
