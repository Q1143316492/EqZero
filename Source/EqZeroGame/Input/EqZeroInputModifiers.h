...
#include "EqZeroInputModifiers.generated.h"

struct FInputActionValue;

class FProperty;
class UEnhancedPlayerInput;
class UEqZeroAimSensitivityData;
class UObject;
...
class UEqZeroSettingBasedScalar : public UInputModifier
{
	GENERATED_BODY()
...
/**
 * This is a deadzone input modifier that will have it's thresholds driven by what is in the EqZero Shared game settings.
 */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "EqZero Settings Driven Dead Zone"))
class UEqZeroInputModifierDeadZone : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class EEqZeroTargetingType : uint8
{
...
/** Applies a scalar modifier based on the current gamepad settings in EqZero Shared game settings. */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "EqZero Gamepad Sensitivity"))
class UEqZeroInputModifierGamepadSensitivity : public UInputModifier
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category=Settings, Config)
	EEqZeroTargetingType TargetingType = EEqZeroTargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, meta=(AssetBundles="Client,Server"))
	TObjectPtr<const UEqZeroAimSensitivityData> SensitivityLevelTable;

protected:
	virtual FInputActionValue ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime) override;
};

/** Applies an inversion of axis values based on a setting in the EqZero Shared game settings */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "EqZero Aim Inversion Setting"))
class UEqZeroInputModifierAimInversion : public UInputModifier
{
	GENERATED_BODY()
