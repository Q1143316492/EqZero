...
#include "EqZeroInputModifiers.h"

#include "EnhancedPlayerInput.h"
#include "GameFramework/PlayerController.h"
#include "Input/EqZeroAimSensitivityData.h"
#include "Player/EqZeroLocalPlayer.h"
#include "Settings/EqZeroSettingsShared.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(EqZeroInputModifiers)

DEFINE_LOG_CATEGORY_STATIC(LogEqZeroInputModifiers, Log, All);

//////////////////////////////////////////////////////////////////////
// EqZeroInputModifiersHelpers

namespace EqZeroInputModifiersHelpers
{
	/** Returns the owning EqZeroLocalPlayer of an Enhanced Player Input pointer */
	static UEqZeroLocalPlayer* GetLocalPlayer(const UEnhancedPlayerInput* PlayerInput)
	{
		if (PlayerInput)
		{
			if (APlayerController* PC = Cast<APlayerController>(PlayerInput->GetOuter()))
			{
				return Cast<UEqZeroLocalPlayer>(PC->GetLocalPlayer());
			}
		}
		return nullptr;
	}
...
//////////////////////////////////////////////////////////////////////
// UEqZeroSettingBasedScalar

FInputActionValue UEqZeroSettingBasedScalar::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	...
	{
		if (UEqZeroLocalPlayer* LocalPlayer = EqZeroInputModifiersHelpers::GetLocalPlayer(PlayerInput))
		{
			const UClass* SettingsClass = UEqZeroSettingsShared::StaticClass();
			UEqZeroSettingsShared* SharedSettings = LocalPlayer->GetSharedSettings();
...
//////////////////////////////////////////////////////////////////////
// UEqZeroInputModifierDeadZone

FInputActionValue UEqZeroInputModifierDeadZone::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	EInputActionValueType ValueType = CurrentValue.GetValueType();
	UEqZeroLocalPlayer* LocalPlayer = EqZeroInputModifiersHelpers::GetLocalPlayer(PlayerInput);
	if (ValueType == EInputActionValueType::Boolean || !LocalPlayer)
	{
		return CurrentValue;
	}

	UEqZeroSettingsShared* Settings = LocalPlayer->GetSharedSettings();
	ensure(Settings);
...
FLinearColor UEqZeroInputModifierDeadZone::GetVisualizationColor_Implementation(FInputActionValue SampleValue, FInputActionValue FinalValue) const
{
...
//////////////////////////////////////////////////////////////////////
// UEqZeroInputModifierGamepadSensitivity

FInputActionValue UEqZeroInputModifierGamepadSensitivity::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	...
	const EEqZeroGamepadSensitivity Sensitivity = (TargetingType == EEqZeroTargetingType::Normal) ? Settings->GetGamepadLookSensitivityPreset() : Settings->GetGamepadTargetingSensitivityPreset();
...
//////////////////////////////////////////////////////////////////////
// UEqZeroInputModifierAimInversion

FInputActionValue UEqZeroInputModifierAimInversion::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	UEqZeroLocalPlayer* LocalPlayer = EqZeroInputModifiersHelpers::GetLocalPlayer(PlayerInput);
	if (!LocalPlayer)
	{
