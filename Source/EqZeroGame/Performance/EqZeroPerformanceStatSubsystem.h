...
#include "EqZeroPerformanceStatSubsystem.generated.h"

enum class EEqZeroDisplayablePerformanceStat : uint8;

class FSubsystemCollectionBase;
class UEqZeroPerformanceStatSubsystem;
class UObject;
struct FFrame;
 ...
 */
class FEqZeroSampledStatCache
{
public:

	FEqZeroSampledStatCache(const int32 InSampleSize = 125)
...
struct FEqZeroPerformanceStatCache : public IPerformanceDataConsumer
{
public:
	FEqZeroPerformanceStatCache(UEqZeroPerformanceStatSubsystem* InSubsystem)
	...
	*/
	double GetCachedStat(EEqZeroDisplayablePerformanceStat Stat) const;
	 ...
	 */
	const FEqZeroSampledStatCache* GetCachedStatData(const EEqZeroDisplayablePerformanceStat Stat) const;

protected:

	void RecordStat(const EEqZeroDisplayablePerformanceStat Stat, const double Value);

	UEqZeroPerformanceStatSubsystem* MySubsystem;

	/**
	 * Caches the sampled data for each of the performance stats currently available
	 */
	TMap<EEqZeroDisplayablePerformanceStat, FEqZeroSampledStatCache> PerfStateCache;
};
...
class UEqZeroPerformanceStatSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
...
protected:
	...
	TSharedPtr<FEqZeroPerformanceStatCache> Tracker;
};
