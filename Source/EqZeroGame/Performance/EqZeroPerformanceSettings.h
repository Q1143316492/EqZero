...
#include "EqZeroPerformanceSettings.generated.h"
...
USTRUCT()
struct FEqZeroQualityDeviceProfileVariant
{
	GENERATED_BODY()
...
USTRUCT()
struct FEqZeroPerformanceStatGroup
{
	GENERATED_BODY()
	...
	UPROPERTY(EditAnywhere)
	TSet<EEqZeroDisplayablePerformanceStat> AllowedStats;
};

// How hare frame pacing and overall graphics settings controlled/exposed for the platform?
UENUM()
enum class EEqZeroFramePacingMode : uint8
{
...
class UEqZeroPlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	UEqZeroPlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const UEqZeroPlatformSpecificRenderingSettings* Get();
...
public:
	...
	// Note that this will usually be set from platform-specific ini files, not via the UI
	UPROPERTY(EditAnywhere, Config, Category=DeviceProfiles)
	TArray<FEqZeroQualityDeviceProfileVariant> UserFacingDeviceProfileOptions;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings)
	EEqZeroFramePacingMode FramePacingMode = EEqZeroFramePacingMode::DesktopStyle;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings, meta=(EditCondition="FramePacingMode==EEqZeroFramePacingMode::MobileStyle", ForceUnits=Hz))
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="EqZero Performance Settings"))
class UEqZeroPerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UEqZeroPerformanceSettings();
...
public:
	...
	TArray<FEqZeroPerformanceStatGroup> UserFacingPerformanceStats;
};
