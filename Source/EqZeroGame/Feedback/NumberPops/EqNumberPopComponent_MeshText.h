...
#include "EqNumberPopComponent_MeshText.generated.h"

class UEqDamagePopStyle;
...
class UEqNumberPopComponent_MeshText : public UEqNumberPopComponent
{
	GENERATED_BODY()

public:

	UEqNumberPopComponent_MeshText(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
...
protected:
	void SetMaterialParameters(const FEqNumberPopRequest& Request, FTempNumberPopInfo& NewDamageNumberInfo, const FTransform& CameraTransform, const FVector& NumberLocation);

	FLinearColor DetermineColor(const FEqNumberPopRequest& Request) const;
	UStaticMesh* DetermineStaticMesh(const FEqNumberPopRequest& Request) const;
	...
	TArray<TObjectPtr<UEqDamagePopStyle>> Styles;
	...
	/** Damage numbers by default are given a depth close to the camera in the material to make sure they are never occluded.
	    This can be toggled off here, should only be 0/1. */
	UPROPERTY(EditDefaultsOnly, Category = "Number Pop|Material Bindings")
