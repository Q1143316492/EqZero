...
#include "EqZeroSettingsLocal.generated.h"

enum class ECommonInputType : uint8;
enum class EEqZeroDisplayablePerformanceStat : uint8;
enum class EEqZeroStatDisplayMode : uint8;

class UEqZeroLocalPlayer;
class UObject;
...
USTRUCT()
struct FEqZeroScalabilitySnapshot
{
	GENERATED_BODY()

	FEqZeroScalabilitySnapshot();
...
/**
 * UEqZeroSettingsLocal
 */
UCLASS()
class UEqZeroSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	static UEqZeroSettingsLocal* Get();
...
public:
	...
	EEqZeroStatDisplayMode GetPerfStatDisplayState(EEqZeroDisplayablePerformanceStat Stat) const;

	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EEqZeroDisplayablePerformanceStat Stat, EEqZeroStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UEqZeroSettingsLocal, FPerfStatSettingsChanged);
	...
	DECLARE_EVENT(UEqZeroSettingsLocal, FLatencyFlashInidicatorSettingChanged);
	UFUNCTION()
	...
	DECLARE_EVENT(UEqZeroSettingsLocal, FLatencyStatEnabledSettingChanged);
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FEqZeroScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FEqZeroScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UEqZeroSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
...
public:
	...
	// Sets the controller representation to use.
	UFUNCTION()
