...
// TODO: [Migration] Ensure GameSubtitles module is included in .Build.cs
...
UENUM(BlueprintType)
enum class EEqZeroColorBlindMode : uint8
{
	Off,
...
UENUM(BlueprintType)
enum class EEqZeroAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
// TODO: [Migration] This header requires ULocalPlayerSaveGame from CommonGame/CommonUser plugin. 
// If appropriate, inherit from ULocalPlayerSaveGame. Otherwise, inherit from USaveGame.
// For now, assuming ULocalPlayerSaveGame is available via CommonGame/CommonUser.
// #include "GameFramework/LocalPlayerSaveGame.h" // Hypothetical path if it were engine
// or check "CommonLocalPlayer.h" or similar.
// In Lyra, ULocalPlayerSaveGame is in CommonUser plugin.
// Since we have CommonUser in Plugins, we can try to use it.
// If not found, fall back to USaveGame.

// Forward declaration if we can't include it right now
class ULocalPlayerSaveGame;

/**
 * UEqZeroSettingsShared - The "Shared" settings are stored as part of the USaveGame system.
 */
UCLASS()
...
public:
	DECLARE_EVENT_OneParam(UEqZeroSettingsShared, FOnSettingChangedEvent, UEqZeroSettingsShared* Settings);
...
public:
	...
	static UEqZeroSettingsShared* CreateTemporarySettings(const ULocalPlayer* LocalPlayer);

	/** Synchronously loads a settings object, this is not valid to call before login */
	static UEqZeroSettingsShared* LoadOrCreateSettings(const ULocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UEqZeroSettingsShared* Settings);
...
public:
	...
	UFUNCTION()
	EEqZeroColorBlindMode GetColorBlindMode() const;
	UFUNCTION()
	void SetColorBlindMode(EEqZeroColorBlindMode InMode);
...
private:
	UPROPERTY()
	EEqZeroColorBlindMode ColorBlindMode = EEqZeroColorBlindMode::Off;
...
public:
	...
	// TODO: [Migration] Re-enable these if ESubtitleDisplayTextSize etc are available (requires GameSubtitles)
	/*
	UFUNCTION()
...
private:
	...
	// TODO: [Migration] GameSubtitles Enums
	/*
	UPROPERTY()
...
public:
	UFUNCTION()
	EEqZeroAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EEqZeroAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EEqZeroAllowBackgroundAudioSetting AllowAudioInBackground = EEqZeroAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EEqZeroGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EEqZeroGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EEqZeroGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EEqZeroGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();

private:
	UPROPERTY()
	EEqZeroGamepadSensitivity GamepadLookSensitivityPreset = EEqZeroGamepadSensitivity::Normal;
	UPROPERTY()
	EEqZeroGamepadSensitivity GamepadTargetingSensitivityPreset = EEqZeroGamepadSensitivity::Normal;
