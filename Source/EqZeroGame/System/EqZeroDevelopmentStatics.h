...
#include "EqZeroDevelopmentStatics.generated.h"
...
UCLASS()
class UEqZeroDevelopmentStatics : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()

public:
	// Should game logic skip directly to gameplay (skipping any match warmup / waiting for players / etc... aspects)
	// Will always return false except when playing in the editor and bTestFullGameFlowInPIE (in EqZero Developer Settings) is false
	...
	// Will always return true except when playing in the editor and bSkipLoadingCosmeticBackgroundsInPIE (in EqZero Developer Settings) is true
	UFUNCTION(BlueprintCallable, Category = "EqZero", meta=(ExpandBoolAsExecs="ReturnValue"))
	...
	EQZEROGAME_API static UWorld* FindPlayInEditorAuthorityWorld();
