...
#include "EqZeroDevelopmentStatics.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EqZeroDevelopmentStatics)

bool UEqZeroDevelopmentStatics::ShouldSkipDirectlyToGameplay()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return !GetDefault<UEqZeroDeveloperSettings>()->bTestFullGameFlowInPIE;
	}
#endif
	return false;
}

bool UEqZeroDevelopmentStatics::ShouldLoadCosmeticBackgrounds()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return !GetDefault<UEqZeroDeveloperSettings>()->bSkipLoadingCosmeticBackgroundsInPIE;
	}
#endif
	return true;
}

bool UEqZeroDevelopmentStatics::CanPlayerBotsAttack()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return GetDefault<UEqZeroDeveloperSettings>()->bAllowPlayerBotsToAttack;
	}
#endif
	return true;
}
...
UWorld* UEqZeroDevelopmentStatics::FindPlayInEditorAuthorityWorld()
{
	check(GEngine);
...
TArray<FAssetData> UEqZeroDevelopmentStatics::GetAllBlueprints()
{
...
UClass* UEqZeroDevelopmentStatics::FindBlueprintClass(const FString& TargetNameRaw, UClass* DesiredBaseClass)
{
	FString TargetName = TargetNameRaw;
	TargetName.RemoveFromEnd(TEXT("_C"), ESearchCase::CaseSensitive);

	TArray<FAssetData> BlueprintList = UEqZeroDevelopmentStatics::GetAllBlueprints();
...
UClass* UEqZeroDevelopmentStatics::FindClassByShortName(const FString& SearchToken, UClass* DesiredBaseClass, bool bLogFailures)
{
